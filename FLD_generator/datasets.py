...
class NLProofSDataset:
    ...
    def generate(self,
        ...
        while i_sample < size:
            ...
            for proof_tree, root_negation_formula, formula_distractors, translation_distractors, others, pipeline_stats in pipeline_results:
                ...
                for proof_stance in self._sample_proof_stances():
                    logger.info('---- sample proof_stance=%s from a logic ----', str(proof_stance))

                    for i_shuffle in range(self._context_shuffles_per_instance):
                        logger.info('---- sample context context shuffle=%d ----', i_shuffle)
                        ...
                        translation_distractors_var = copy.deepcopy(translation_distractors)
                        others_var = copy.deepcopy(others)
                        pipeline_stats_var = copy.deepcopy(pipeline_stats)

                        # -- sample stance --
                        if len(proof_tree_var.leaf_nodes) == 0:
                            # For some very rare case, this occurs.
                            # Since we do not expect this behaviour, we raise error for future debug
                            raise Exception(proof_tree_var.format_str)
                        ...
                        context, formula_context, proof_text, formula_proof_text, node2id, id2node = self._make_text(
                            ...
                            formula_distractors=formula_distractors_var,
                            translation_distractors=translation_distractors_var,
                        ...
                        all_negative_tree_attrs = [val for name, val in others_var.items()
                        ...
                        all_negative_formulas = formula_distractors_var
                        ...
                        if stance_msg is not None:
                            ...
                            logger.fatal(pformat(proof_tree_var.leaf_nodes))
                            ...
                            logger.fatal(proof_tree_var.format_str)
                        ...
                        dataset_json = {
                            ...
                            'num_translation_distractors': len(translation_distractors_var),
                            'num_all_distractors': len(formula_distractors_var) + len(translation_distractors_var),
                        }
                        ...
                        # Update statistics
                        if self.log_stats:
                            sample_stats = flatten_dict(pipeline_stats_var)
    ...
    def _sample_proof_stances(self) -> List[ProofStance]:
        if self._sample_all_stances_per_logic:
            return self.proof_stances
        else:
            # TODO: to be with self.proof_stances
