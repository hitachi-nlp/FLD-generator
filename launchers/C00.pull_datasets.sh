...
output_dir=./outputs/C00.pull_datasets.sh/20230801.case_study_finalize.fix/FLD.v2/FLD.3
mkdir -p ${output_dir} 1>/dev/null 2>&1
for split in "train" "valid" "test"; do
  rsync -av es.abci.ai:${source_ABIC_dir}/${split}/${split}.jsonl ${output_dir}
done
...
output_dir=./outputs/C00.pull_datasets.sh/20230801.case_study_finalize.fix/FLD.v2/FLD.4
mkdir -p ${output_dir} 1>/dev/null 2>&1
for split in "train" "valid" "test"; do
  rsync -av es.abci.ai:${source_ABIC_dir}/${split}/${split}.jsonl ${output_dir}
done
